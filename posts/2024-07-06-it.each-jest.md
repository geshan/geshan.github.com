...
Below is an example of the tagged template literal format for Jest each with the above math power example:
...
* **Picked up by courier** - The order and its items are in a box and are with the courier.
* **Out for Delivery** - The parcel is being sent to the customer by the courier.
...
* **Processing** - equivalent to the above internal statuses from `Created` to `Sent to courier`.
* **Order shipped** - equivalent to the above internal statuses of `Picked up by courier` and `Out for Delivery`.
...
The above code snippet defines a const `statusMap`  that has keys for all the different internal order statuses. Then we define a `getPublicOrderStatus` function that takes the internal order status as a parameter and returns the user-friendly order status that will be visible to the customer. It uses a Map with an Object that has the internal order status as the key and the customer friendly order status as the value.
...
As you can see above, the code seems to be very repetitive. Next, you are going to use `Jest each` to write the code for all the internal to customer status mapping.
...
Here the test cases look much simpler and streamlined. The `orderStatuses` array is taken in as the input for `it.each` and for each row the test runs once. If you run the test it will give the following output:
...
Jest `each` is a great feature to write concise and easy-to-read tests. It also helps you write data-driven tests with more code coverage with less test code. Make it a habit to use `each` in your Jest tests and I hope it will make you a better software engineer. Keep writing tests!
