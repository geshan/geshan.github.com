...
---
Gemini by Google is a powerful LLM. In this tutorial, you will use the Gemini Pro 1.0 version to summarize text from a URL. In the previous [part](https://geshan.com.np/blog/2024/02/gemini-vertex-ai-nodejs/) you learned how to set up a GCP project, try out the prompt on Vertex AI studio and then try to code on the CLI. In this part, you will refactor and convert that code to take in a URL, scrape its content, and use it to create the summary, let's get going!
...
The things that have changed here are that you have renamed the `getContent` function to `getSummary`. Then you have also removed the static text that had news about removing waste from waterways to a new variable called `text`. The `text` variable is passed in as a parameter to the `getSummary` function which makes it dynamic. Then you expose the `getSummary` with the `module.exports` so that it can be used with `require` in any other file and get the returned summary.
...
That is the contents of the page and it returns it. In case of any error, it logs the error and returns an empty string. To test this out, you will create a new `cli.js` that uses both the scraper and Gemini files next.
...
Here, you first require the `scraper.js` and the `gemini.js`  as `summaryGenerator`. Then you have an async [IIFE](https://developer.mozilla.org/en-US/docs/Glossary/IIFE) - Immediately Invoked Function Expression which parses the URL from the CLI parameters. If the URL is not provided, it falls back to a News post from 7News. Then it prints the URL it is going to use for scraping and ultimately generating a summary.

After that, it sends the URL to the scraper’s `getContents` function to get the contents of that web page. If there is no content (even due to an error), it returns `Could not get data from the given URL` else it sends the page contents to the `getSummary` function in the `gemini.js` file to get the summary.
...
Now the summarizer is working in the CLI. If you don’t provide a URL, it will fall back to the 7News luggage URL. There is a two-second timeout for fetching the page, so it will throw an error for slow websites. There is no validation of the URL, and other things can be made better too. In the next section, you will see how it works as an API.
...
Then, you log the URL to see which web page is being scraped. Subsequently, you scrape the content of that URL's web page by passing the URL to the scraper's `getContents` function. It will return the contents of the page if things go fine; otherwise, it will return an empty string. Returning an empty string will translate into a 501 response with the message `Could not get data properly from the given URL`. This can happen if the URL takes more than 2 seconds to respond or if the data cannot be easily parsed by the scraper.
...
You can run it with `node index.js`. For local development, you can also use [nodemon](/blog/2021/02/nodemon/). You can also add it as the `start` script in the `package.json` file’s `scripts` section as highlighted below:
...
After that, if you open your favorite browser and navigate to `http://localhost:3000/summary?url=https://developer.mozilla.org/en-US/docs/Glossary/IIFE`, you will see something like the following (if everything went well):
...
There you have it: you have successfully converted the sample code from the Vertex AI 'Get Code' section into a working API. You can easily deploy it to a service like Cloud Run on GCP, with or without dockerizing the app. If you push your code to GitHub, you can also use the [Cloud Shell Editor](/blog/2024/01/cloud-shell-editor/) to clone and deploy the code to Cloud Run, which can be a topic for another blog post. To deploy this code on Cloud Run, some values will need to be dynamic or picked from environment variables, such as the GCP project and GCP location.
...
You can also create an [e-commerce product description generator](/blog/2024/04/gemini-ecommerce-product-description-generator/) using Gemini over Vertex AI that takes in pictures and a prompt to give out a decent product description. Try it.

## Conclusion

In part 2 of this tutorial, you were able to convert the sample code into a full-fledged API. You also have a quick way to test out any URL using the modified `cli.js` file. Here is [part 1](/blog/2024/02/gemini-vertex-ai-nodejs/).
