...
Testing code that interacts with the native `Date` object can be challenging because the `Date` object is inherently dynamic. Its value changes with every passing millisecond, making it difficult to write deterministic tests. Imagine you have a function that shows the day of the year. If you run this function today (on September 26th), it will show 270 (out of 365), but if you run it tomorrow, it will show 271 (out of 365). This means the test cannot be repeatable unless the system date is mocked and set to a defined date.

Furthermore, the native `Date` object is a global object in JavaScript, meaning that any changes made to it within one test can potentially affect other tests, leading to unexpected and inconsistent results. This can make debugging and troubleshooting your tests very hard. So how can we solve this issue? Enter fake timers and setting the system time in Jest 26+, which is discussed next.
...
It has a YTD option that shows the price from January 2nd to today. It will be updated every day. So, your task is to get the number of the day in the year.
...
It looks long, but nothing complicated is happening in the above code. It was originally a one-liner on a Stack Overflow answer; it has been modified to look like the above. 

First, you get today’s system date with `new Date()`, then you convert today’s date to milliseconds. Similarly, you convert the start of the year’s date to milliseconds. Then, you subtract those two by doing today’s date in milliseconds - start of the year in milliseconds. After that, you convert the difference, which is in milliseconds, to days by dividing it by `1000 * 60 * 60 * 24`, which has been illustrated in 3 lines to make it clearer. With this, you get a number, for example, `270` if you run the above code on September 26th.
...
The above code was run on September 26th. Next, you will write repeatable tests for the above `getDayOfYear` function that will pass not only today, but on any day of the year.

## Test for the day of the year example

Now, to write a unit test for the above code with Jest, it is straightforward. The issue is if you run the test tomorrow (September 27th), your assertion of the 270th day of the year fails. Since the date changes every day, you could write a test checking that the day of the year is greater than 0 and less than 366, but such a test is quite useless.

This is where the power of mocking the date, or say stubbing it to be one particular date, comes in very handy. To do this, you can use the following unit test code:
...
Keep this test file in the `test` folder and name it `dateHelper.test.js`. The `dateHelper.js` file should be in the `src` folder.

First, you import the `getDayOfYear` function from the module; yes, this is an ES6 import (not the CommonJS require). Then you write the test code. The main point to consider here is that in the `beforeEach` section, you stub/mock the date to January 7th, 2024. To do this, you use Jest's fake timers. With these timer mocks, Jest can swap out timers with functions that allow you to control the passage of time. Then you set the system time to be January 7th, 2024. That is exactly what you need for your tests to be repeatable and reliable.

In the `afterEach` section for these tests, you reset the system time to be the actual time, and you also use real timers for the other tests outside that describe block. You certainly don’t want to mess up other tests outside that describe block or even in that test file. 
...
Now, whether you run it today, tomorrow, or any day of the year, it will still pass. Why? Because for that test, the system date is always fixed to January 7th, 2024, as the date has been mocked with fake timers and setting the system time in Jest to mock the real date.
...
There are other options to mock the date in Jest. If you are using a Jest version of less than 26, you can use Jest SpyOn to mock the global Date object. It is neither easy nor is the code clean, but that is a way of doing it. Frankly, you should be using Jest 26+ as version 26 was released more than 4 years ago in May 2020.

You can also use the `jest-mock-date` NPM package, which has 270K+ downloads each week at the time of writing this. But if you can do it with Jest and fake timers/set system date, I think that option is far better.
