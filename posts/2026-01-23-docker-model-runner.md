...
Below are some prerequisites you will need installed on the machine you want to run Docker Model Runner (DMR) on:
...
You can run the following command to pull [Smollm2](https://hub.docker.com/r/ai/smollm2) from DockerHub:
...
After that, you can run the Smollm2 135-million-parameter model as discussed next.
...
<img class="center" src="/images/docker-model-runner/05dmr-chat-with-smollm.jpg" loading="lazy" title="Docker model runner running Smollm2 and user having a chat with it" alt="Docker model runner running Smollm2 and user having a chat with it">
...
You can look at all the prompts given to the model on Docker Desktop by clicking the model in the `Models` screen, which is `Smollm2` in this case:
...
<img class="center" src="/images/docker-model-runner/08docker-desktop-listing-prompts.jpg" loading="lazy" title="Listing run prompts on Desktop Models section Requests tab" alt="Listing run prompts on Desktop Models section Requests tab">
...
You can also inspect the model’s architecture, parameters, and other information in the `Inspect` tab:
...
In this quick and useful tutorial, you learned how to pull an open model like Smollm2 from DockerHub and run it on your local machine. This is just scratching the surface; with Docker Model Runner, you can run many open models on your machine, from Gemma to Llama, and from Qwen to DeepSeek depending on your hardware. Keep learning!
