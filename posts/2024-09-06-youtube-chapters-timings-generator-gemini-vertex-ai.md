...
It is no secret that YouTube [video chapters](https://support.google.com/youtube/answer/9884579?hl=en) are a huge time saver for your audience. Be it a long 2-hour podcast or a 40-minute talk video, if the video has relevant chapters, your audience can jump to the parts that are most meaningful for them.

Creating chapter data manually is a tedious and time-consuming task. You will need to sit down with something to take notes, be it pen and paper or your mobile phone’s notes app. Then play the video and move the progress bar here and there to note down the timings from the video to list them down as chapters. What if there was an automated and easier way to do this? Why can’t an LLM like Gemini, which now has a 2 million context window, do it?
...
2. Make sure you have selected the project created in Step 1 if you have multiple projects. You can do this by clicking on the project name in the top-left corner of your Google Cloud Console.
...
4. Click on `Vertex AI`. On the Vertex AI page, click "Agree & Continue" as seen below:
...
5. After that, on the Vertex AI page, click on “Enable All Recommended APIs” as follows (it will take some time):
...
9. After that, paste the URL of a video that is on your channel and then click the `Validate` button; once the ownership is verified, you can `Insert` the video as follows:
...
10. Given your YouTube video is included in the prompt, it will look like the one below:
...
11. At this point, you can save your prompt for further use. To save it, click on `Save` in the top-right corner and give it a name like `yt-timings`:
...
12. Let’s look into the configuration a bit. First, select `gemini-1.5-flash-001` as the model. If you want the chapter timings to be less random, you can set the temperature to `0.1`:

<img class="center" loading="lazy" src="/images/youtube-chapters-timings-generator/11vertex-model-temp.jpg" title="Select Gemini Flash and set temperature to 0.1" alt="Select Gemini Flash and set temperature to 0.1">
...
14. I am not covering safety settings for this tutorial. Now, you can click on the `>` button to see what chapter timings Gemini Flash generates for your video; keep in mind that, depending on the length of the video, it will take time and also use up your Google Cloud Credit:

<img class="center" loading="lazy" src="/images/youtube-chapters-timings-generator/13chapters-timings-generated.jpg" title="Prompt on Vertex AI with Gemini Flash worked to generate YouTube Video chapters' timings" alt="Prompt on Vertex AI with Gemini Flash worked to generate YouTube Video chapters' timings">
...
This is the video I used for this tutorial, and I did edit the chapters' information a bit to make it better. You can delete the video from the prompt and link another YouTube video, then generate the chapter information for that one too.

Amazing! There you have a fully reusable YouTube video chapter timings generator using your own YouTube video. For all the experiments I did to generate video chapter timings using Gemini Flash over Vertex AI, it cost me 35 cents. Google Cloud credits are provided for this project and blog post, thanks to Google for that. It is part of the #AISprint.

As LLMs give out probabilistic output rather than deterministic, the output is a good starting point. As a human, you will need to cross-check and edit the output to fit your needs. The same thing applies to the chapter information generated by Gemini.

I used it for a podcast video and interestingly enough it can generate chapter information even when there are no visual queues in the video. So, it can use the transcribed captions as well for generating the chapter information. This was the [podcast video](https://www.youtube.com/watch?v=hU3rjY0MRVc) I used to generate chapters.

You can also click "Get Code" and play around with it; you can look at an example of generating and running the code in this [example](/blog/2024/04/gemini-ecommerce-product-description-generator/#run-generated-python-code) with Gemini and Vertex AI.

## Conclusion

In this blog post, you created a YouTube video chapter timings generator using Gemini Flash over Vertex AI. If your videos are longer, you can use Gemini Pro, which has a 2 million context window, which is more expensive than the Flash version.
