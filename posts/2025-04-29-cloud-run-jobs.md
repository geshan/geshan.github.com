...
* The script executes with the `main` call on line 115, which has a catch attached to it. It would have been easier with a [JavaScript try catch](/blog/2024/06/javascript-try-catch/) block, but this is the official example. So, let’s just roll with it.
* Before the `main` function on line 68, four environment variables are taken out of the `process.env` with object destructuring. Those are `CLOUD_RUN_TASK_INDEX` and `CLOUD_RUN_TASK_ATTEMPT`, which are used to determine the task's position within the job and the retry attempt (if the job has failed). 
...
* The main task (in the job, which is the `main` function), which logs the task number and the attempts first.
* Then it waits for the specified milliseconds and then randomizing the failure according to the failure rate.
...
After that, click on the `Cloud Shell` icon beside the bell icon on the top right (or press g and s, on your keyboard). The Cloud Shell will appear at the bottom of the screen and ask for permission. Click `Authorize`:
...
It will take some minutes for the code to be copied to a bucket and then built with build packs on Google Cloud Build. If you want to follow the build process, you can open the visible URL on a new browser tab, it will look something like this:
...
<img class="center" src="/images/cloud-run-jobs/15cloud-run-jobs-ran.jpg" loading="lazy" title="Cloud Run Jobs ran successfully after being triggered from Cloud Shell" alt="Cloud Run Jobs ran successfully after being triggered from Cloud Shell">
...
<img class="center" src="/images/cloud-run-jobs/16cloud-scheduler-command.jpg" loading="lazy" title="Cloud scheduler command to create new schedule" alt="Cloud scheduler command to create new schedule">
...
<img class="center" src="/images/cloud-run-jobs/17cloud-scheduler-page.jpg" loading="lazy" title="Cloud scheduler page showing newly created schedule" alt="Cloud scheduler page showing newly created schedule">
...
In this post, you learned about Google Cloud Run Jobs and their usage. You also learned how to create and run Cloud Run jobs on a schedule with Google Cloud Scheduler with a simple Node.js Example. I hope you can utilize Cloud Run Jobs to execute tasks efficiently. Keep learning!
