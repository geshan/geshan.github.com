...
JavaScript has a `Date` object representing a single moment in time in a platform-independent format. This is achieved by representing the date object as a number of milliseconds since January 1st, 1970 UTC. 
...
* **getTime() Method:** The `getTime()` method returns the number of milliseconds since the Unix epoch (seconds that have elapsed since 00:00:00 UTC on January 1st, 1970), making it reliable for comparing dates.
...
One issue that can happen here is that if a date is created with `new Date()` and another date is created with `new Date()` after two seconds, those two dates will not match. Also, time zones will be another consideration to make for comparing date objects.
...
For a reliable date comparison in JavaScript, it is recommended to use the `getTime()` method available on the `Date` object. This is because when you compare `Date` objects directly, they are compared based on references, not their actual value. Even with the `getTime()` method, you will need to consider time zones; the easiest way to do it will be to convert all the dates to the UTC time zone before doing the date/time comparison. 
...
- Even with `getTime`, do keep the time zone issue in mind (e.g., [5 AM in London is 4 PM in Sydney](https://www.worldtimebuddy.com/?qm=1&lid=2147714,2643743&h=2147714&date=2024-11-1&sln=16-16.5&hf=1)), and also think about date changes and daylight saving time.

The example below shows how to compare dates using the `getTime()` method to make it more reliable:
...
Even here you are comparing "dates," not date-times (meaning `2024-11-01 3:00:00` is after `2024-11-01 2:58:58`); consider this fact. Also, keep in mind the time zone differences.

In the next section, you will learn about a very popular JavaScript library called `date-fns` and how to use it to compare dates with ease.
...
Let's analyze the above code. First, you import the compareAsc function from date-fns using ESM imports. Then, you initialize three date objects date1, date2, and date3. Then you run different comparisons between the three dates; you also do a sorting of the 3 dates in ascending order. When you run the above script, the output looks like the following:
...
* `format`: for formatting the date as per the needed locale.
...
These are some of the many other useful functions `date-fns` has; you can find a complete list in the [date-fns documentation](https://date-fns.org/docs/Getting-Started). Use the search function to make the most of it.

All the code examples are available in this [GitHub repository](https://github.com/geshan/javascript-compare-dates). If you want to write effective unit tests for dates, you will need to [mock dates in Jest](/blog/2024/09/jest-mock-date/) to get repeatable and predictable results.
